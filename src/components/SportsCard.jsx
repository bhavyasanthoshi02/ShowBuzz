...
const SportsCard = ({ event, favorites, toggleFavorite }) => {
  const navigate = useNavigate();
  const isFavorite = favorites?.some((m) => m.title === event.title);

  return (
    <div className="movie-card sports-card" onClick={() => navigate(`/sports-detail/${event.id}`)} style={{cursor: "pointer"}}>
      ...
      <div className="movie-card-img-wrapper">
        <img
          ...
          alt={event.title}
          onError={(e) => {
            e.target.src = "https://via.placeholder.com/300x450?text=Sport+Event";
          }}
        />
          ...
          <button 
            ...
            onClick={(e) => {
              ...
              if (event.status !== "Sold Out" && event.status !== "Upcoming") {
                navigate(`/book-sport/${event.id}`);
              }
            }}
          >
            {event.status === "Sold Out" ? "Sold Out" : event.status === "Upcoming" ? "Stay Tuned" : "Book Seats"}
          </button>
        </div>
      </div>
