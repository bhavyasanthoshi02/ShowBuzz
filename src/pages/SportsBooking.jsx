...
const SEAT_LAYOUT = {
  "VIP":      { price: 2000, rows: ["A","B"],             cols: 10, gap: [5] },
  "PREMIUM":  { price: 1000, rows: ["C","D","E","F"],      cols: 15, gap: [5, 10] },
  "GALLERY":  { price: 500,  rows: ["G","H","I","J","K"],  cols: 20, gap: [10] },
  "GENERAL":  { price: 200,  rows: ["L","M","N","O","P"],  cols: 20, gap: [5, 15] },
};
...
export default function SportsBooking() {
  ...
  // 1=Stadium Info, 2=Seats, 3=Summary
  ...
  useEffect(() => {
    const found = sportsData.find(s => s.id === id);
  ...
  const startCollabSession = () => {
    const sid = "session_sport_" + Math.random().toString(36).substr(2, 9);
    setSessionId(sid);
    setIsCollab(true);
    setStep(2);
  };
  ...
  const finalizeBooking = () => {
    const bookingId = "SP" + Date.now().toString().slice(-8);
  ...
  const applyCoupon = () => {
    ...
    const sportSpend = saved.filter(b => b.isSport).reduce((sum, b) => sum + b.totalAmount, 0);
    const points = Math.floor(saved.reduce((sum, b) => sum + b.totalAmount, 0) / 10);
    
    let disc = 0;
    let code = couponCode.toUpperCase();
    
    if (code === "SPORT50" && sportSpend > 500) {
      disc = 50;
      setAppliedCoupon({ code, discount: "₹50 Off" });
      ...
      <div className="bk-topbar">
        ...
        <div className="bk-steps">
           <div className={`bk-step ${step >= 1 ? "active" : ""} ${step > 1 ? "done" : ""}`}>
             ...
             <span className="bk-step-label">Event Info</span>
           </div>
            ...
            <div className="bk-theater-card selected">
               <div className="bk-theater-header">
                  ...
                  <div className="bk-theater-meta">
                    <MapPin size={14} /> {event.venue.split(",")[1] || "Arena"} • {event.date}
                  </div>
                  <div className="bk-theater-tags">
                    <span className="bk-tag">Stadium Entry</span>
